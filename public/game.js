...
function stopGrowJob() {
  if (gameState.growJob) {
    clearTimeout(gameState.growJob);
    gameState.growJob = null;
  }
}
...
function drawCircle(x,y,r,color="#ff0000") {
  ctx.beginPath();
  ctx.fillStyle = color;
...
function drawHitEffect(x,y,text) {
  drawFrame();
...
function colorForRadius(r) {
  const ratio = Math.min(1, r / RADIO_MAXIMO);
  const rx = Math.floor(Math.min(255, 255*ratio*2));
  const gx = Math.floor(Math.max(0, 255 - 255*ratio*2));
  return `rgb(${rx},${gx},0)`;
}
