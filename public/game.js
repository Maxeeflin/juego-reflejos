...
socket.on("room_update", (room) => { if(!room) return; myRoom=room; updateRoomUI(); });
socket.on("game_started", ({ code }) => { statusDiv.innerText="Partida iniciada — cuenta atrás..."; startLocalPlay(); });
socket.on("game_over", ({ results }) => { showFinalResults(results); });

// ----------------- UI actions -----------------
btnCreate.addEventListener("click", () => {
  const name = nameInput.value.trim() || "Host";
  const numPlayers = prompt("Modo: 2 o 3 jugadores?","2");
  const capacity = (numPlayers==="3")?3:2;
  socket.emit("create_room",{numPlayers:capacity,name},(res)=>{
    if(res.ok){ myRoom=res.room; gameState.roomCode=res.code; isHost=true;
      statusDiv.innerText=`Sala creada: ${res.code} (esperando jugadores...)`; updateRoomUI(); btnStart.style.display="inline-block";
...
btnJoin.addEventListener("click", () => {
  ...
  socket.emit("join_room",{code,name},(res)=>{
    ...
    myRoom=res.room; gameState.roomCode=code; isHost=false;
    statusDiv.innerText=`En la sala ${code} — esperando start`; updateRoomUI();
  });
});
...
btnStart.addEventListener("click", () => {
  ...
  socket.emit("start_game",{code:myRoom.code},(res)=>{
    ...
    statusDiv.innerText="Partida iniciada"; btnStart.style.display="none";
  });
});
...
function updateRoomUI(){
  ...
  if(myRoom.hostSocketId===mySocketId && !myRoom.started) btnStart.style.display="inline-block";
...
function startLocalPlay(){
  ...
  drawClear(); drawCenteredText("Toca para iniciar (pantalla)",20);
  canvas.onclick=()=>{canvas.onclick=null; startCountdown(CUENTA_ATRAS_SEG,()=>startRound());};
  statusDiv.innerText="Tu turno: empieza en breve"; updateRoomUI();
}

function startCountdown(n, cb){
  let cur=n;
  const tick=()=>{
    drawClear(); drawCenteredText(String(cur),80);
    if(cur===0) cb&&cb(); else { cur--; setTimeout(tick,1000); }
  };
  tick();
}
...
function stopGrowJob(){ if(gameState.growJob){ clearTimeout(gameState.growJob); gameState.growJob=null; } }
...
gameState.growLoop=function growLoop(){
  ...
  if(gameState.radio>=RADIO_MAXIMO){
    stopGrowJob(); gameState.rondaEnCurso=false; gameState.rondasPerdidas+=1;
    drawExplosion(gameState.x,gameState.y,gameState.radio);
    setTimeout(()=>{ onRoundEnded(); },1000); return;
  }
  drawFrame();
...
function onCanvasClick(evt){
  ...
  if(dist<=gameState.radio){
    ...
    if(gameState.vidas<=0){
      stopGrowJob(); gameState.rondasPerdidas+=1; gameState.rondaEnCurso=false;
...
function onRoundEnded(){
  ...
  if(gameState.rondasPerdidas>=RONDAS_POR_JUGADOR){
    ...
    socket.emit("player_finished",{code:gameState.roomCode||myRoom.code,totalPoints:gameState.puntos},(res)=>{
      if(res && res.ok){ statusDiv.innerText="Resultados enviados. Esperando a los demás..."; canvas.onclick=null; }
    ...
    canvas.onclick=null;
    startCountdown(5,()=>{ startRound(); statusDiv.innerText=`Jugando - Puntos: ${gameState.puntos}`; });
  }
}
...
function drawFrame(){ drawClear(); drawCircle(gameState.x,gameState.y,gameState.radio,colorForRadius(gameState.radio)); drawHearts(); ctx.fillStyle="white"; ctx.font="16px Arial"; ctx.textAlign="left"; ctx.fillText(`Rondas perdidas: ${gameState.rondasPerdidas}/${RONDAS_POR_JUGADOR}`,10,30); ctx.fillText(`Puntos: ${gameState.puntos}`,10,50); }
function drawHearts(){ const hearts="❤️".repeat(gameState.vidas)+"💔".repeat(Math.max(0,VIDAS_FUERA-gameState.vidas)); ctx.font="22px Arial"; ctx.textAlign="right"; ctx.fillText(hearts,CANVAS_WIDTH-10,30); }
function drawExplosion(x,y,r){ drawCircle(x,y,r,"#000"); ctx.strokeStyle="#ff6666"; for(let i=0;i<6;i++){ ctx.beginPath(); ctx.arc(x,y,r+i*6,0,Math.PI*2); ctx.stroke(); } }
...
function showFinalResults(results){
  ...
  cont.style.color="white"; cont.style.textAlign="center";
  cont.innerHTML=`<h2>Estadísticas finales</h2>`;
  results.forEach((r)=>{ const p=document.createElement("div"); p.style.fontSize="20px"; p.style.margin="8px"; p.innerText=`${r.name}: 0`; cont.appendChild(p); });
  const btn=document.createElement("button");
  btn.innerText="Reiniciar partida";
  btn.onclick=()=>{
    finalScreen.style.display="none";
    socket.emit("restart_game",{code:gameState.roomCode},(res)=>{
      if(res.ok){
        gameState.velocidad=VELOCIDAD_INICIAL;
        gameState.radio=RADIO_INICIAL;
        gameState.vidas=VIDAS_FUERA;
